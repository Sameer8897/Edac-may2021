...
select (rpad(REGION_NAME,40,'*'),80,'*') from countries;
select concat(lpad(REGION_NAME,((80+length(REGION_NAME))/2),'*'),rpad('',((80-length(REGION_NAME))/2),'*')) from regions;
...
update employees
	set SALARY=2400, COMMISSION_PCT=5
...
select `PHONE_NUMBER` from employees where  'NULL';
...
INSERT INTO `job_history`countries (`EMPLOYEE_ID`, `START_DATE`, `END_DATE`, `JOB_ID`, `DEPARTMENT_ID`) VALUES
